...
/** @type {typeof import("webpack") | undefined} */
let webpackPeer;
try {
  webpackPeer = (await import("webpack")).default;
} catch {
  // webpack is an optional peer dependency
}
...
class Server {
  ...
  addAdditionalEntries(compiler) {
    ...
    const webpack =
      compiler.webpack ||
      /** @type {NonNullable<typeof webpackPeer>} */ (webpackPeer);
    ...
    switch (typeof clientTransport) {
      case "string":
        ...
        if (clientTransport === "ws") {
          ...
          try {
            clientImplementation = fileURLToPath(
              import.meta.resolve(clientTransport),
            );
          } catch {
  ...
  async createServer() {
    ...
    if (typeof type === "function") {
      ...
      const mod = await import(/** @type {string} */ (type));
